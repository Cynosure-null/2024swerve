      ...
      magnet_offset{magnet_offset}
{
    ...
    cancoder_config.MagnetSensor.SensorDirection.value = 1; // QUESTION: why this and not not CCW_Positive?
    // cancoder_config.MagnetSensor.SensorDirection.CounterClockwise_Positive = 1;
    cancoder_config.MagnetSensor.MagnetOffset = magnet_offset;
    ...
    // turner_config.Slot0.kP = 0.00025;
    ...
    turner_config.Feedback.SensorToMechanismRatio = TURNER_GEAR_RATIO;
...
units::degree_t SwerveModule::getAngle() { return units::degree_t{cancoder.GetAbsolutePosition().Refresh().GetValue()}; }
...
void SwerveModule::setDesiredState(frc::SwerveModuleState const &desired_state)
{
    ...
    frc::SmartDashboard::SmartDashboard::PutNumber(driver.GetDescription() + "/desired angle", optimized_angle.Degrees().value());
    driver.SetControl(controls::VelocityDutyCycle{bot_speed_to_wheel_speed(optimized_speed)});
    turner.SetControl(controls::PositionDutyCycle{optimized_angle.Degrees()});
